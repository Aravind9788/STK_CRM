...
interface LeadQuotationPayload {
    ...
    material_name?: string; // Added optional
    brand?: string; // Added optional
    ...
    advance_received?: number; // Advance paid
    payment_mode_handover: string; // <--- The new field we need to show
...
const StoreOrderDetailsScreen = ({ route, navigation }: any) => {
    ...
    const fetchOrders = useCallback(async () => {
        try {
            ...
            // Call the endpoint
            const response = await fetch(`${SERVER_URL}/store-manager/fetch-pending-leads/${lead_id}`);
                ...
                >
                    ...
                    <View style={styles.card}>
                        ...
                        <InfoRow
                            ...
                            value={`₹${(order.advance_received || 0).toLocaleString()}`}
                            icon="cash-check"
                        />
                        <InfoRow
                            label="Collected By"
                        ...
                        {/* === NEW FIELD: BALANCE COLLECTED SITE === */}
                        <InfoRow
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <Text style={{ fontSize: 18, fontWeight: '800', color: PRIMARY_COLOR }}>
                                ₹{((order.amount || 0) - (order.advance_received || 0)).toLocaleString()}
                            </Text>
                        </View>
                    </View>
                    ...
                    {order.status === 'DISPATCHED' && (
                        <View style={styles.actionCard}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={async () => {
                                    // 1. Create the full payload with Timestamp
                                    const payload = {
                                        order_id: order.id,
                                        feedback: feedback,
                                        payment_status: paymentStatus,

                                        // ✅ NEW: Capture exact time of click
                                    ...
                                    if (await markOrderAsDelivered(payload)) {
                                        ...
                                        // 3. Update Local State (So the app updates instantly)
                                        updateOrder({
                                            ...order,
                                            status: 'DELIVERED',
                                            feedback: feedback,
