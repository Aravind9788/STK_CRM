...
const AddTeamLeadScreen = ({ navigation }: any) => {
    const [fullName, setFullName] = useState('');
    const [password, setPassword] = useState('');
    const [storeAssigned, setStoreAssigned] = useState('');
    const [teamLeads, setTeamLeads] = useState<StaffListItem[]>([]);
    ...
    const fetchTeamLeads = async () => {
        try {
            ...
            const response = await fetch(`${SERVER_URL}/director-dashboard/team-leads`, {
                // headers: {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch team leads:', response.status, errorText);
                Alert.alert('Error', `Failed to fetch team leads: ${response.status}`);
            }
        } catch (error) {
    ...
    const handleCreateTeamLead = async () => {
        ...
        try {
            ...
            const response = await fetch(`${SERVER_URL}/director-dashboard/create-team-lead`, {
                method: 'POST',
                headers: {
                    // 'Authorization': `Bearer ${token}`,
            ...
            if (response.ok) {
                ...
                Alert.alert('Error', errorData.detail || 'Failed to create team lead');
            }
        } catch (error) {
    ...
    const previewId = storeAssigned.trim()
        ? `TL-${getLocationCode(storeAssigned)}-XXX`
        : 'TL-XXX-XXX';
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle}>Add Team Lead</Text>
                <View style={{ width: 24 }} />
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
                ...
                <View style={styles.formCard}>
                    <Text style={styles.sectionTitle}>Create New Team Lead</Text>
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                ...
                                <Text style={styles.createButtonText}>Create Team Lead</Text>
                            </>
                        )}
                    </TouchableOpacity>
                </View>
                ...
                <View style={styles.tableCard}>
                    <Text style={styles.sectionTitle}>Existing Team Leads</Text>
                    ...
                    {fetchingData ? (
                        ...
                        <Text style={styles.emptyText}>No team leads created yet</Text>
                    ) : (
                        <View style={styles.table}>
                            ...
                            {teamLeads.map((teamLead) => (
                                <View key={teamLead.id} style={styles.tableRow}>
                                    <Text style={[styles.tableCell, { flex: 1.5 }]} numberOfLines={1}>{teamLead.staff_id}</Text>
                                    <Text style={[styles.tableCell, { flex: 2 }]} numberOfLines={1}>
                                        {teamLead.full_name}
                                    </Text>
                                    <Text style={[styles.tableCell, { flex: 1.5 }]} numberOfLines={1}>
                                        {teamLead.store_assigned}
                                    </Text>
                                    <Text style={[styles.tableCell, { flex: 1.5 }]} numberOfLines={1}>
                                        {teamLead.password}
                                    </Text>
...
export default AddTeamLeadScreen;
